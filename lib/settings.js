...
>>>>>>> 810734d5643250bb85f3dea169192960f054bb38
function getProfileName(argv, env, pkgName) {
  let i = argv.findIndex(v=> /-p|--profile/.test(v) )
  if ( i >=  0 ) return argv[i+1]
  return env[`${pkgName.toUpperCase()}_PROFILE`]  || 'default' 
}

const profileName = getProfileName(process.argv, process.env, name) 
...
async function load(cloud = null ) {
  ...
  if (cloud) config.cidata = await cidata.load(config)
  return config
}
...
//   program.debug && console.error('cmd:',cmd,'\nk: ', k,'\nv: ', v, '\noptions: ', options)
//   let o = config
...
//       break;
//     case 'get' : 
//       if ( o[k] ) console.log(JSON.stringify(o[k]))
//      //if (o[k] ) process.stdout.write(JSON.stringify(o[k]) + "\n",'utf8')
//       break;
//     case 'delete' :
//       if (! config[k])  break;
//       delete o[k]
...
async function validate(config, cloud = {} ) {
  if ( typeof(cloud) == 'undefined' || Object.keys(cloud).length == 0 )  return config
>>>>>>> 810734d5643250bb85f3dea169192960f054bb38
