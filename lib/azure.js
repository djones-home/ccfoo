...
function tableOutput({data}) {
  ...
  data.forEach( e => {
    t.cell('##', i )
    //t.cell('UUID', e.id)
    var { publisher: p, offer: o, sku: s, version: v   } = e.storageProfile.imageReference

    t.cell('name', e.name )
    t.cell('location', e.location )
    t.cell('vmSize', e.hardwareProfile.vmSize) 
    t.cell('image', [p,o,s,v].join('-'))
    t.cell('GB', e.StorageProfile.osDisk.diskSizeGB)
    if (e.StorageProfile.managedDisk)
      t.cell("storType", e.StorageProfile.managedDisk.storageAccountType)
