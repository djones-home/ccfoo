...
async function getCreds(config) {
  ...
  const password =
    ...
    config.AZURE_PASSWORD ||
    config.password ||
    '';

  //let creds = await tokens.get(loginOptions)
  config.creds = await msRestAzure.loginWithUsernamePassword(
    username,
    password,
    loginOptions
  );
...
async function getVMlist(program, config) {
  let ep =
    msRestAzure.AzureEnvironment[config.environmentName]
...
async function getResourcesList(program, config) {
  let ep =
    msRestAzure.AzureEnvironment[config.environmentName]
      .resourceManagerEndpointUrl;
  const creds = await getCreds(config);
  let client = new resourceManagement.ResourceManagementClient(
    creds,
    config.id,
    ep
  );
...
async function filterResources({ program, config, ...rest }) {
  ...
  if (program.verbose > 0)
    console.error(
      `level-${program.verbose}: ${subject} count: ${df.length} of ${
        d.length
      } ${subject}s W/ filter :`,
...
function tableOutput({ data }) {
  ...
  data.forEach(e => {
    ...
    t.cell(
      'Name',
      e.value.universalName || e.value.namespace || urlParse(e.value.url).path
    );
...
async function show({ program, subject, config, ...rest }) {
  console.log('YTBD');
}
async function launch({ program, subject, config, ...rest }) {
  console.log('YTBD');
}
async function stop({ program, subject, config, ...rest }) {
  console.log('YTBD');
}
async function start({ program, subject, config, ...rest }) {
  console.log('YTBD');
}
async function terminate({ program, subject, config, ...rest }) {
  console.log('YTBD');
}
