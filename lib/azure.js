...
const rm = require('azure-arm-resource')
...
async function getNetwork({ program, config }) {
  let cId = cache.id('getNetwork', config)
...
async function deploy({ program, config, template, parameters, resource_group }) {
  let endPoint = msRestAzure.AzureEnvironment[config.environmentName].resourceManagerEndpointUrl
  const creds = await getCreds({ config })
  let client = new rm.ResourceManagementClient(creds, config.id, endPoint)
  // await createResourceGroup(client)
  //client.deployments.createOrUpdate(rgName, depName,
  //return await client.resources.list()
...
async function getResources({ program, config }) {
  ...
  let cId = cache.id('getResource',config)
  let d = cache.cGet(cId, program.ttl)
  if (!d) {
...
async function launch({ program, subject, config, ...rest }) {
  ...
  if (!program.Name) {
    log("Name ?: ", cidata.listVm({ data: config.cidata }).join(' '))
    return
  }
  let roleData = cidata.getVm({ data: config.cidata, name: program.Name })
  log("Paramters ", program.Name, roleData)

  let temptemplate = require('../templates/arm/vm/tempate')

  //let template = fs.readFileSync( role.template ) || require(`../templates/${config.provider}/vm`)
  await deploy({ program, config, temptemplate, parameters, resource_group })
...
async function validateConfig(p) {
  ...
  if (!p.environmentName) p.environmentName = 'AzureUSGovernment'
  ...
  p.environment = AzureEnvironment[p.environmentName];
  //p.resourceManagerEndpointUrl = p.environment.resourceManagerEndpointUrl
  return p
}

function createResourceClient(credentials, config, subject = null) {
  const { id, environment } = config;


  const promise = new Promise((resolve, reject) => {
    // Create an ARM client
    var client = new rm.ResourceManagementClient(
      credentials,
      id,
      environment.resourceManagerEndpointUrl
    );
    if (client) {
      log('got the ResourceManagementClient client');
      resolve(client);
    } else {
...
function createResourceGroup(client, config, subject) {
  const promise = new Promise((resolve, reject) => {
    // Use the RM client to create a resource group
    const name = config.cidata.Project.Name + subject;
    const properties = {
      location: config.location
    };

    client.resourceGroups.createOrUpdate(name, properties, (err, response) => {
      if (err) reject(err);

      let group = response;
      log(group);
      log(`Created Resource Group named ${group.name}`);

      resolve({ client, group });
    });
  });
  log('returning promise createResourceGroup');
  return promise;
}

function deployTemplate({ client, group, template, parameters }) {

  const deploymentName = 'STEVE1';
  ...
  const promise = new Promise((resolve, reject) => {
    // Deploy template
    console.error(`Start Deployed Shared Resources ARM Template`);
    client.deployments.createOrUpdate(group.name, deploymentName, deploymentParameters, (err, result) => {
      if (err) reject(err);

      log(`Done Deployed Shared Resources ARM Template`);

      // Remove the Type attribute for the RG outputs
      var scrubbedOutputs = _.mapValues(result.properties.outputs, output => {
  ...
  log('returning promise deploySharedTemplate');
  return promise;
}

function establishResourceGroup(azCreds, config, subject = null) {
  return createResourceClient(azCreds, config, subject).then(client => {
    return createResourceGroup(client, config, subject)
  })
}

function mapToAzureVMParameters(target, roleData) {
  let result = target || {};
  ...
  var azParams = {
    ...
    virtualMachineSize: '',
    virtualNetworkName: '',
    identity: x=> { return x.IamInstanceProfile },
    virtualMachineName: x => { return x.Subnet },
    steve: x => {}
  }

  let names = Object.keys(azParams)
  names.forEach(key => {
    let value = undefined;
    let functOrName = azParams[key];

    if (typeof functOrName === "function") {
      value = functOrName(roleData);
    } else if ( functOrName ) {
      value = roleData && roleData[functOrName];
    }
    if (value) {
      result[key] = value;
    }
...
module.exports = {
  ...
  getNetwork,
  establishResourceGroup,
  deployTemplate,
  mapToAzureVMParameters
}
