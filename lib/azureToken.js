...
let MyTokenCache = (function() {
  function MyTokenCache(name, filename) {
  ...
  MyTokenCache.prototype.save = function(done) {
    ...
    fs.writeFileSync(this.getFilename(), JSON.stringify(this.tokens), writeOptions, done);
  };
...
function useCasheCredentials(profile) {
  const promise = new Promise((resolve, reject) => {
...
function getCredentials(profile) {
  ...
  let filename = profile.credentialsStore.az;
  tokenCache = tokenCache || new MyTokenCache(name, filename);

  if (tokenCache.empty()) {
    return interactiveLogin();
  } else {
    return useCasheCredentials(profile);
  }
}
