...
function validate({config, data, ...opt}) {
   if ( data.Schema && (data.Schema == currentSchema) || (data.Schema == '20170226') ) {
     return data
   }
   throw new Error("validate cidata failes")
}
...
function normalize({data, config, ...opt}) {
   ...
   switch(data.Schema) {
     case  currentSchema :
       return data
     case '20170226':
       return translate20170226({data})
     default :
        throw new Error("Unknown Schema.")
   }
}

function getVm({data, name, config, ...opt}) {
  return profiles.getRole({ subject: 'Vm', data: normalize({data}), name, config })
} 
...
function translate20170226({data}) {
  ...
  console.error(`WARNING No CIDATA translation for ${data.Schema}`)
  return data
