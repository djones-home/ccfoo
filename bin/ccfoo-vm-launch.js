...
async function listResource() {
  ...
  if (program.verbose > 0)
    console.error(
      `level-${program.verbose}: ${subject} count: ${
        d.length
...
async function getCreds(config) {
  ...
  const loginOptions = {
    environment: msRestAzure.AzureEnvironment[config.cloudName]
  };
  const username =
...
async function rmDeploy(config, creds) {
  ...
  let endpoint =
    msRestAzure.AzureEnvironment[config.cloudName].resourceManagerEndpointUrl;
  const client = new resourceManagement.ResourceManagementClient(
    creds,
    config.id,
    endpoint
  ...
  await client.deployments.createOrUpdate(
    rgName,
    deploymentName,
